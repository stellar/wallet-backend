...
This prerequisite is natural given that [[mutations do not submit transactions — clients receive signed XDR and submit directly to Stellar network]] — the signed inner XDR from `BuildAndSignTransaction` is precisely what the client holds before calling `FeeBumpTransaction`. The two mutations form a sequential pair: build-and-sign first, fee-bump second.
