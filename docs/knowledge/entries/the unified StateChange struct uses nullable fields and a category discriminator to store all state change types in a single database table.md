...
This design was chosen over a polymorphic table design (one table per category with a JOIN to a parent) because the read pattern is predominantly by address+time rather than by category. A unified table allows a single index on `(address, created_at)` to serve all category queries. The tradeoff is nullable columns and the need for category-aware application code.
