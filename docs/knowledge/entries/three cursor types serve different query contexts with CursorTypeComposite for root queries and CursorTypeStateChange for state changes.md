...
At the SQL layer, each cursor type determines which columns appear in the decomposed cursor condition:
- **CursorTypeComposite** for transactions: `(ledger_created_at, to_id)` — `(ledger_created_at > $1) OR (ledger_created_at = $1 AND to_id > $2)`
- **CursorTypeComposite** for operations: `(ledger_created_at, id)` — same structure with operation `id`
- **CursorTypeStateChange**: `(ledger_created_at, to_id, op_id, sc_order)` — four-column decomposition

The decomposed form (not ROW tuple comparison) is required for TimescaleDB chunk pruning. See [[decomposed OR cursor condition enables TimescaleDB chunk pruning whereas ROW tuple comparison is opaque to the columnar scan engine]] for the performance rationale.
