...
- [[wazero pure-go wasm runtime validates sep-41 contract specs in-process by compiling wasm and parsing the contractspecv0 custom section]] (performance tradeoff accepted), [[checkpoint population disables FK checks via session_replication_role replica to allow balance entries to be inserted before their parent contract rows]] (superuser required), and [[contract metadata fetching rate-limits to 20 parallel contracts per batch with 2s sleep to avoid overloading stellar rpc during checkpoint]] (hardcoded constants) together imply: "PopulateAccountTokens() has three distinct operator-configuration risks — a performance cliff from wazero, a permission failure for FK-disable, and a throughput ceiling from hardcoded rate-limiting. Each fails in a different way: one degrades silently, one fails explicitly at runtime, one throttles without feedback." Not yet captured as a standalone entry.
...
- 2026-02-26: session_replication_role=replica appears twice in the codebase at different scopes — [[backfill mode trades acid durability for insert throughput via synchronous_commit off]] applies it connection-wide for all backfill writes; [[checkpoint population disables FK checks via session_replication_role replica to allow balance entries to be inserted before their parent contract rows]] applies it LOCAL to a single transaction (auto-reverts at COMMIT). When debugging FK errors in either context, scope is the key diagnostic dimension.
