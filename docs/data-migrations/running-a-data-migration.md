...
When a bug fix changes what a protocol derives, the rows already in the database stay wrong: current-state columns like SEP-41 balances and Blend cost basis are running totals that no on-chain query can re-derive. `--rebuild` discards the protocol's rows and replays them through the fixed code.
