	...
	tests := []struct {
		...
		},
		{
			...
			columns: []CursorColumn{
				...
				{Name: "state_change_id", Value: int64(30)},
			},
			sortOrder:     DESC,
			startArgIndex: 2,
			wantClause:    "(to_id < $2 OR (to_id = $3 AND operation_id < $4) OR (to_id = $5 AND operation_id = $6 AND state_change_id < $7))",
		...
		},
		{
			...
			sortOrder:     DESC,
			startArgIndex: 1,
			wantClause:    "(ledger_created_at < $1 OR (ledger_created_at = $2 AND to_id < $3) OR (ledger_created_at = $4 AND to_id = $5 AND operation_id < $6) OR (ledger_created_at = $7 AND to_id = $8 AND operation_id = $9 AND state_change_id < $10))",
		...
		},
		{
			...
			sortOrder:     ASC,
			startArgIndex: 5,
			wantClause:    "(ledger_created_at > $5 OR (ledger_created_at = $6 AND to_id > $7) OR (ledger_created_at = $8 AND to_id = $9 AND operation_id > $10) OR (ledger_created_at = $11 AND to_id = $12 AND operation_id = $13 AND state_change_id > $14))",
			wantArgs:      []interface{}{testTime, testTime, int64(10), testTime, int64(10), int64(20), testTime, int64(10), int64(20), int64(30)},
			wantNextIndex: 15,
		},
	}
