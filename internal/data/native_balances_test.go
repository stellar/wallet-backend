...
func TestNativeBalanceModel_GetByAccount(t *testing.T) {
	...
	cleanUpDB := func() {
		_, err = dbConnectionPool.Pool().Exec(ctx, `DELETE FROM native_balances`)
		require.NoError(t, err)
	}
...
func TestNativeBalanceModel_BatchUpsert(t *testing.T) {
	...
	t.Run("inserts new native balance", func(t *testing.T) {
		...
		// Verify insert
		var count int
		err = dbConnectionPool.Pool().QueryRow(ctx, `SELECT COUNT(*) FROM native_balances WHERE account_address = $1`, "GACCOUNT1").Scan(&count)
		require.NoError(t, err)
	...
	t.Run("handles combined upserts and deletes", func(t *testing.T) {
		...
		// Verify results
		var count int
		err = dbConnectionPool.Pool().QueryRow(ctx, `SELECT COUNT(*) FROM native_balances`).Scan(&count)
		require.NoError(t, err)
