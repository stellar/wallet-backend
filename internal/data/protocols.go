...
func (m *ProtocolsModel) UpdateCurrentStateMigrationStatus(ctx context.Context, dbTx pgx.Tx, protocolIDs []string, status string) error {
	...
	_, err := dbTx.Exec(ctx, query, status, protocolIDs)
	if err != nil {
		m.MetricsService.IncDBQueryError("UpdateCurrentStateMigrationStatus", "protocols", utils.GetDBErrorType(err))
		return fmt.Errorf("updating current state migration status for protocols: %w", err)
	}

	m.MetricsService.ObserveDBQueryDuration("UpdateCurrentStateMigrationStatus", "protocols", time.Since(start).Seconds())
	m.MetricsService.IncDBQuery("UpdateCurrentStateMigrationStatus", "protocols")
	return nil
}
