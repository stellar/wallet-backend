...
func TestTrustlineAssetModel_BatchGetByIDs(t *testing.T) {
	...
	t.Run("returns assets for valid IDs", func(t *testing.T) {
		...
		var assetIDs map[string]int64
		err = db.RunInPgxTransaction(ctx, dbConnectionPool, func(dbTx pgx.Tx) error {
			assetIDs, err = m.BatchGetOrInsert(ctx, dbTx, assets)
			return err
		})
		require.NoError(t, err)
...
func TestTrustlineAssetModel_BatchGetOrInsert(t *testing.T) {
	...
	t.Run("returns empty map for empty input", func(t *testing.T) {
		...
		err = db.RunInPgxTransaction(ctx, dbConnectionPool, func(dbTx pgx.Tx) error {
			result, err = m.BatchGetOrInsert(ctx, dbTx, []TrustlineAsset{})
			return err
		})
		require.NoError(t, err)
	...
	t.Run("creates single new asset and returns ID", func(t *testing.T) {
		...
		var result map[string]int64
		err = db.RunInPgxTransaction(ctx, dbConnectionPool, func(dbTx pgx.Tx) error {
			result, err = m.BatchGetOrInsert(ctx, dbTx, assets)
			return err
		})
		require.NoError(t, err)
	...
	t.Run("returns existing IDs for already existing assets (fast path)", func(t *testing.T) {
		...
		var result1 map[string]int64
		err = db.RunInPgxTransaction(ctx, dbConnectionPool, func(dbTx pgx.Tx) error {
			result1, err = m.BatchGetOrInsert(ctx, dbTx, assets)
			return err
		})
		require.NoError(t, err)
		...
		err = db.RunInPgxTransaction(ctx, dbConnectionPool, func(dbTx pgx.Tx) error {
			result2, err = m.BatchGetOrInsert(ctx, dbTx, assets)
			return err
		})
		require.NoError(t, err)
	...
	t.Run("handles mix of existing and new assets", func(t *testing.T) {
		...
		var result1 map[string]int64
		err = db.RunInPgxTransaction(ctx, dbConnectionPool, func(dbTx pgx.Tx) error {
			result1, err = m.BatchGetOrInsert(ctx, dbTx, initialAssets)
			return err
		})
		require.NoError(t, err)
		...
		var result2 map[string]int64
		err = db.RunInPgxTransaction(ctx, dbConnectionPool, func(dbTx pgx.Tx) error {
			result2, err = m.BatchGetOrInsert(ctx, dbTx, mixedAssets)
			return err
		})
		require.NoError(t, err)
...
func TestTrustlineAssetModel_BatchInsert(t *testing.T) {
	...
	t.Run("inserts single asset and returns ID", func(t *testing.T) {
		cleanUpDB()
		mockMetricsService := metrics.NewMockMetricsService()
	...
	t.Run("inserts multiple assets and returns IDs", func(t *testing.T) {
		cleanUpDB()
		mockMetricsService := metrics.NewMockMetricsService()
		mockMetricsService.On("ObserveDBQueryDuration", "BatchInsert", "trustline_assets", mock.Anything).Return()
		mockMetricsService.On("IncDBQuery", "BatchInsert", "trustline_assets").Return()
	...
	t.Run("returns same IDs for duplicate insert (idempotent)", func(t *testing.T) {
		cleanUpDB()
		mockMetricsService := metrics.NewMockMetricsService()
		mockMetricsService.On("ObserveDBQueryDuration", "BatchInsert", "trustline_assets", mock.Anything).Return()
		mockMetricsService.On("IncDBQuery", "BatchInsert", "trustline_assets").Return()
	...
	t.Run("handles mix of new and existing assets", func(t *testing.T) {
		cleanUpDB()
		mockMetricsService := metrics.NewMockMetricsService()
		mockMetricsService.On("ObserveDBQueryDuration", "BatchInsert", "trustline_assets", mock.Anything).Return()
		mockMetricsService.On("IncDBQuery", "BatchInsert", "trustline_assets").Return()
