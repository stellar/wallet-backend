...
func TestStateChangeModel_BatchCopy_DuplicateFails(t *testing.T) {
	...
	// Pre-insert the state change using direct SQL
	_, err = dbConnectionPool.ExecContext(ctx, `
	...
	`, sc1.ToID, sc1.StateChangeOrder, string(sc1.StateChangeCategory), string(*sc1.StateChangeReason), sc1.LedgerCreatedAt, sc1.LedgerNumber, sc1.AccountID, sc1.OperationID)
	require.NoError(t, err)
