...
func TestOperationModel_GetAll(t *testing.T) {
	...
	operations, err := m.GetAll(ctx, "", nil, nil, true)
	require.NoError(t, err)
	...
	limit := int32(2)
	operations, err = m.GetAll(ctx, "", &limit, nil, true)
	require.NoError(t, err)
...
func TestOperationModel_BatchGetByAccountAddresses(t *testing.T) {
	...
	operations, err := m.BatchGetByAccountAddress(ctx, address1, "", nil, nil, true)
	require.NoError(t, err)
	assert.Len(t, operations, 2)
	assert.Equal(t, int64(2), operations[0].Operation.ID)
	assert.Equal(t, int64(1), operations[1].Operation.ID)
}
