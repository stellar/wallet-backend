...
func Test_IngestStoreModel_GetLatestLedgerSynced(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			key:  "ingest_store_key",
			setupDB: func(t *testing.T) {
				_, err := dbConnectionPool.Pool().Exec(ctx, `INSERT INTO ingest_store (key, value) VALUES ($1, $2)`, "ingest_store_key", 123)
				require.NoError(t, err)
			},
			expectedLedger: 123,
		},
...
func Test_IngestStoreModel_UpdateLatestLedgerSynced(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err = m.DB.Pool().QueryRow(ctx, `SELECT value FROM ingest_store WHERE key = $1`, tc.key).Scan(&dbStoredLedger)
			require.NoError(t, err)
...
func Test_IngestStoreModel_UpdateMin(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err = dbConnectionPool.Pool().Exec(ctx, `INSERT INTO ingest_store (key, value) VALUES ($1, $2)`, tc.key, tc.initialValue)
			require.NoError(t, err)
