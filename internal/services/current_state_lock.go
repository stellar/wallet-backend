...
// by every exclusive protocol-migrate run: current-state migrations and
// --rebuild invocations of either strategy. Live ingestion is deliberately not
// a party — the per-ledger cursor CAS already decides which writer folds a
// given ledger. Same fnv64a construction as the live-ingest lock
...
// acquireCurrentStateLocks try-locks every protocol's advisory lock on one
// dedicated connection held for the run, and returns the release function to
// defer. A held lock means another migration or rebuild owns that protocol
// right now — the caller must not proceed.
func acquireCurrentStateLocks(ctx context.Context, pool *pgxpool.Pool, protocolIDs []string) (func(), error) {
	...
	for _, pid := range protocolIDs {
		...
		if !acquired {
			destroyConn()
			return nil, fmt.Errorf("current-state lock for protocol %q is held: another migration or rebuild is running", pid)
		}
