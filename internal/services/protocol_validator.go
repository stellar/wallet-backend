...
//  2. Prefetch: resolves on-chain enrichment (e.g. SEP-41 token metadata,
//     Blend pool config) via RPC. Called before any transaction opens.
//  3. Apply: persists the protocol's side effects (e.g. contract_tokens,
//     blend_pools rows) inside the caller's dbTx, using only what Prefetch
...
type ProtocolValidator interface {
	...
	// carry (e.g. Blend's pool-vs-backstop role). Called before any
	...
	// contract_tokens, blend_pools rows) inside dbTx, using only the plan
