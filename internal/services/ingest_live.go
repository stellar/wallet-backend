...
// concurrently, in commit order. The first five own the bulk-COPY tables
// (data.BulkCopyTables, which startup reconciliation reads to clear their
// orphans); the last two own the mutable balance families and commit last,
// right before the coordinating transaction that carries the cursor: a
// stranded bulk row can be deleted, but a balance row can only be overwritten
// by replay, so committing those groups last keeps the window in which such a
// row is visible ahead of the cursor as short as the barrier allows.
// Test_persistSiblings_Order pins the order. stateChangesMu serializes the
// state_changes transaction, which the coordinating goroutine also writes
// protocol history through — see protocolHistorySink and the chunk-boundary
// deadlock note in persistLedgerData.
...
//
// Channel depth is bounded by ledger count, not bytes, and scales with the
// persist batch cap. Oversized ledgers are the axis to watch under a memory
// limit.
//
...
func (m *ingestService) ingestLiveLedgers(ctx context.Context, startLedger uint32, checkLockSession func(ctx context.Context) error) error {
	...
	// from turning the hand-off into a rendezvous — the queue slot is what
	// lets process start the next ledger while persist commits this one.
	processed := make(chan processedLedger, max(1, batchCap-1))
	...
	// batch carrying it commits, so sustaining a full batch needs batchCap
	// buffers for the in-flight batch, cap(processed) for the queue, and one
	// for process to fill — 2*batchCap+1 covers every batch cap, and the
	// channel's capacity matches the buffer count so handing one back never
	// blocks. Sizing the rotation any tighter caps the batch below batchCap
	// no matter how deep the backlog — process runs out of buffers before the
	// queue can refill, and the batch settles at the size where held and
	// refillable buffers balance.
