...
//
// auct_type is rejected above math.MaxInt32: it is stored as an int32, so a
// larger value would wrap negative and persist a row no reader can map back to
// an auction type. Every value that does fit is kept — the current protocol
// defines 0..2, and an auction type added by a future protocol version must be
// stored rather than dropped (GraphQL skips the ones it cannot name).
func decodeAuctionKey(elems []xdr.ScVal, kind string) (auctType uint32, user string, err error) {
	...
	if auctType > math.MaxInt32 {
		return 0, "", fmt.Errorf("blend: %s: auction auct_type %d exceeds int32 range", kind, auctType)
	}
