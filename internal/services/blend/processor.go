...
// state) and ContractEvents (Blend history) into staged sets, then
...
type processor struct {
	...
	// decodeFailures counts events/entries whose key or topic symbol this
	// package recognizes but whose payload fails to decode — probable contract
...
func newProcessor(deps services.ProtocolDeps) *processor {
	...
	if deps.MetricsService != nil {
		p.decodeFailures = deps.MetricsService.Ingestion.ProtocolDecodeFailuresTotal
	}
	p.Reset()
	return p
}
...
// countDecodeFailure records one dropped event ("event") or ContractData entry
// ("entry") whose recognized symbol failed to decode. It is a no-op when the
// processor was built without a metrics service.
func (p *processor) countDecodeFailure(kind string) {
	if p.decodeFailures == nil {
		return
	}
	p.decodeFailures.WithLabelValues(ProtocolID, kind).Inc()
}

// StateChangeOrdinalBase returns the Blend state_change_id namespace base.
func (p *processor) StateChangeOrdinalBase() int64 { return types.StateChangeOrdinalBaseBlend }
...
// carry Blend history rows and cost-basis fold instructions, but never the
// current-state snapshot itself.
func (p *processor) RequiresContractData() bool { return true }

// ProcessLedger folds this ledger's Blend contract events (Blend history
...
func (p *processor) ProcessLedger(_ context.Context, input services.ProtocolProcessorInput) error {
	...
	// Iterate event groups in on-chain (tx, op) order, not map order: unlike
	// history rows (whose ordinals are per-group and immune to iteration
	// order), the cost-basis folds accumulate across groups into shared
	// staged state, and ZeroBorrowed (bad_debt) makes that fold destructive —
	// a borrow and a bad_debt for the same (pool, user, asset) in different
	// groups of one ledger must fold in chain order to net correctly.
	keys := make([]indexer.ContractEventKey, 0, len(input.ContractEvents))
	for key := range input.ContractEvents {
	...
	slices.SortFunc(keys, func(a, b indexer.ContractEventKey) int {
		...
		return cmp.Compare(a.OpIdx, b.OpIdx)
	})
	for _, key := range keys {
		...
		for _, event := range events {
			if err := p.processEvent(event, opBuilder, input.StagingMode, blndToken); err != nil {
				...
				// The topic symbol is one this package models, so a decode failure
				// means the emitting contract's shape drifted from the decoder; the
				// event is dropped, so surface it loudly.
				p.countDecodeFailure("event")
				log.Warnf("blend: skipping malformed event at tx=%d op=%d: %v", key.TxIdx, key.OpIdx, err)
				continue
			}
		}
	}
...
//
// The deltas stay in protocol-token units here; ApplyAuctionAdjustments values
// them at the b_rate/d_rate persisted for the reserve when the window commits,
// so every fill in a multi-ledger window is converted at end-of-window rates
// rather than at its own ledger's rates. The divergence from per-ledger
// ingestion is bounded by the interest that accrues between a fill and window
// close — relative error at most borrow APR × (window duration / year), ~1e-5
// for the default 100-ledger (~8 min) window at 50% APR. It touches only the
// interestEarned/interestPaid cost-basis display fields and applies once, to
// windowed migration: live ingestion runs window=1, where the conversion is
// exact.
...
func (p *processor) processContractDataChanges(changes map[string][]ingest.Change) {
	for addr, entries := range changes {
		...
		for _, change := range entries {
			...
			if err != nil {
				// The key symbol is one this package models, so a decode failure
				// means the owning contract's storage shape drifted from the
				// decoder; the entry is dropped, so surface it loudly.
				p.countDecodeFailure("entry")
				log.Warnf("blend: skipping malformed contract data entry on %s: %v", addr, err)
				continue
			}
...
func (p *processor) PersistHistory(ctx context.Context, dbTx pgx.Tx) error {
	...
	// the Blend base so they can never collide with the main indexer's or
	// SEP-41's IDs for the same operation (see types.AssignStateChangeOrdinals).
...
// persistReserves upserts full reserve rows for reserves whose config AND
// data halves were both staged this window, and partially updates just the
// staged half otherwise. See stagedReserve's godoc for why the halves are
...
func (p *processor) persistReserves(ctx context.Context, dbTx pgx.Tx) error {
	...
	var configRows []blenddata.ReserveConfigUpdate
	for key, sr := range p.stagedReserves {
		if sr.config != nil && sr.data == nil {
			// ResData wasn't observed alongside this window's new/changed
			// ResConfig. In practice set_reserve always writes both entries
			// together, so this is defensive: a config-only partial update
			// leaves the live rate/supply columns untouched, degrading an
			// invariant violation to a stale data half (or, for a reserve
			// with no row yet, a skip until its ResData arrives) instead of
			// overwriting real rates with zeros.
			configRows = append(configRows, blenddata.ReserveConfigUpdate{
				Pool:         key.Pool,
				Asset:        key.Asset,
	...
	if len(configRows) > 0 {
		if err := p.reserves.BatchUpdateConfig(ctx, dbTx, configRows); err != nil {
			return fmt.Errorf("updating %d blend reserve config rows for ledger %d: %w", len(configRows), p.ledgerNumber, err)
		}
	}
	return nil
}
