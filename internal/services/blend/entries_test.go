...
func TestDecodeEntry_Auction(t *testing.T) {
	...
	t.Run("auct_type at the int32 boundary is stored, not dropped", func(t *testing.T) {
		got, err := DecodeEntry(createdChange(auctionKey(math.MaxInt32), val))
		require.NoError(t, err)

		assert.Equal(t, KindAuction, got.Kind)
		assert.Equal(t, int32(math.MaxInt32), got.AuctionType)
	})

	t.Run("auct_type above int32 range is rejected", func(t *testing.T) {
		_, err := DecodeEntry(createdChange(auctionKey(math.MaxInt32+1), val))
		require.Error(t, err)
