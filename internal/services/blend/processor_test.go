...
// scKind is a staged state change's full identity under the entity-first
// encoding: the category names the on-chain object and the reason the action
// on it. Neither half identifies a row on its own — CREDIT, for instance,
// appears under BLEND_SUPPLY, BLEND_COLLATERAL and BLEND_BACKSTOP alike.
type scKind struct {
	category types.StateChangeCategory
	reason   types.StateChangeReason
}

// groupByKind buckets staged state changes by (category, reason), preserving
// emission order within each bucket.
func groupByKind(changes []types.StateChange) map[scKind][]types.StateChange {
	out := map[scKind][]types.StateChange{}
	for _, sc := range changes {
		k := scKind{sc.StateChangeCategory, sc.StateChangeReason}
		out[k] = append(out[k], sc)
	}
	return out
}
...
func TestProcessLedger_CountsDecodeFailures(t *testing.T) {
	...
	// A recognized "supply" topic whose data vec is one element short, and a
	// recognized "ResConfig" key whose value is not a config map: both shapes
	// this package models, both undecodable.
	badEvent := contractEvent(t, poolAddr,
		[]xdr.ScVal{symScVal("supply"), contractAddrScVal(t, asset), accountAddrScVal(t, user)},
		vecScVal(i128ScVal(100)),
	)
	badEntry := createdChange(vecScVal(symScVal("ResConfig"), contractAddrScVal(t, asset)), voidScVal())

	require.NoError(t, p.ProcessLedger(ctx, services.ProtocolProcessorInput{
		LedgerSequence:      7,
		ProtocolContracts:   []data.ProtocolContracts{protocolContractFor(t, poolAddr, "aa")},
		ContractEvents:      map[indexer.ContractEventKey][]xdr.ContractEvent{{TxIdx: 0, OpIdx: 0}: {badEvent}},
		ContractDataChanges: map[string][]ingest.Change{poolAddr: {badEntry}},
		StagingMode:         services.StagingModeBoth,
	}))

	counter := appMetrics.Ingestion.ProtocolDecodeFailuresTotal
	assert.Equal(t, 1.0, testutil.ToFloat64(counter.WithLabelValues(ProtocolID, "event")))
	assert.Equal(t, 1.0, testutil.ToFloat64(counter.WithLabelValues(ProtocolID, "entry")))
	assert.Empty(t, p.stagedStateChanges)
	assert.Empty(t, p.stagedReserves)
}

// TestProcessLedger_DecodeFailuresWithoutMetrics pins the nil-metrics guard:
// processors built without a metrics service still drop malformed input
// instead of panicking on the counter.
func TestProcessLedger_DecodeFailuresWithoutMetrics(t *testing.T) {
	ctx := context.Background()
	p := newProcessor(services.ProtocolDeps{NetworkPassphrase: testNetworkPassphrase})
	require.Nil(t, p.decodeFailures)
	...
	require.NoError(t, p.ProcessLedger(ctx, services.ProtocolProcessorInput{
		LedgerSequence:    7,
		ProtocolContracts: []data.ProtocolContracts{protocolContractFor(t, poolAddr, "aa")},
		ContractEvents: map[indexer.ContractEventKey][]xdr.ContractEvent{{TxIdx: 0, OpIdx: 0}: {
			contractEvent(t, poolAddr,
				[]xdr.ScVal{symScVal("supply"), contractAddrScVal(t, asset), accountAddrScVal(t, user)},
				vecScVal(i128ScVal(100)),
			),
		}},
		ContractDataChanges: map[string][]ingest.Change{poolAddr: {
			createdChange(vecScVal(symScVal("ResConfig"), contractAddrScVal(t, asset)), voidScVal()),
		}},
		StagingMode: services.StagingModeBoth,
	}))
	assert.Empty(t, p.stagedStateChanges)
...
func TestProcessLedger_StagingModes(t *testing.T) {
	...
	t.Run("current-state mode stages current state but no history", func(t *testing.T) {
		...
		// Sanity: the current-state side does reach the data layer. The input
		// staged ResConfig without ResData, so the reserve goes through the
		// config-only partial update, not a full-row upsert.
		m.reserves.On("BatchUpdateConfig", mock.Anything, mock.Anything, mock.Anything).Return(nil).Once()
...
func TestProcessLedger_BackstopClaimAutoRestake(t *testing.T) {
	...
	byKind := groupByKind(p.stagedStateChanges)

	claims := byKind[scKind{types.StateChangeCategoryBlendBackstopEmissions, types.StateChangeReasonClaim}]
	...
	_, hasSource := claimSC.KeyValue["source"]
	assert.False(t, hasSource, "the claim's source is carried by the category, not an Extra key")
	...
	deposits := byKind[scKind{types.StateChangeCategoryBlendBackstop, types.StateChangeReasonCredit}]
	require.Len(t, deposits, 2)
	depositByPool := map[string]types.StateChange{}
	for _, sc := range deposits {
...
func TestProcessLedger_StagesHistory(t *testing.T) {
	...
	requireOne := func(category types.StateChangeCategory, reason types.StateChangeReason) types.StateChange {
		t.Helper()
		k := scKind{category, reason}
		require.Len(t, byKind[k], 1, "expected exactly one %s/%s row", category, reason)
		return byKind[k][0]
	}

	// supply: tx0/op0 -> ToID = toid.New(300,0,0), OperationID = toid.New(300,0,1).
	supplySC := requireOne(types.StateChangeCategoryBlendSupply, types.StateChangeReasonCredit)
	...
	borrowSC := requireOne(types.StateChangeCategoryBlendDebt, types.StateChangeReasonBorrow)
	assert.Equal(t, wantTxID, borrowSC.ToID) // same tx as supply
	assert.Equal(t, wantOpID, borrowSC.OperationID)

	backstopWithdrawSC := requireOne(types.StateChangeCategoryBlendBackstop, types.StateChangeReasonDebit)
	...
	claimSC := requireOne(types.StateChangeCategoryBlendBackstopEmissions, types.StateChangeReasonClaim)
	...
	badDebtSC := requireOne(types.StateChangeCategoryBlendDebt, types.StateChangeReasonBadDebt)
	...
	fillKind := scKind{types.StateChangeCategoryBlendAuction, types.StateChangeReasonFill}
	require.Len(t, byKind[fillKind], 2)
	for _, sc := range byKind[fillKind] {
	...
	// PersistHistory numbers each (tx, op) group's rows 1..N in emission order
	// within Blend's state_change_id namespace: supply and borrow share tx0/op0
	// and get distinct ordinals, while the fill_auction pair — alone in tx3/op2 —
	// restarts at 1.
	persisted := groupByKind(p.stagedStateChanges)
	assert.Equal(t, types.StateChangeOrdinalBaseBlend+1,
		persisted[scKind{types.StateChangeCategoryBlendSupply, types.StateChangeReasonCredit}][0].StateChangeID)
	assert.Equal(t, types.StateChangeOrdinalBaseBlend+2,
		persisted[scKind{types.StateChangeCategoryBlendDebt, types.StateChangeReasonBorrow}][0].StateChangeID)
	fills := persisted[fillKind]
	assert.Equal(t,
		[]int64{types.StateChangeOrdinalBaseBlend + 1, types.StateChangeOrdinalBaseBlend + 2},
		[]int64{fills[0].StateChangeID, fills[1].StateChangeID})
}
...
// TestProcessLedger_FoldsEventGroupsInOpOrder pins ProcessLedger's cross-group
// fold order for a SINGLE ledger: input.ContractEvents is a map, and the
// ZeroBorrowed (bad_debt) fold is destructive on the shared per-(pool, user,
// asset) accumulator, so groups must fold in (TxIdx, OpIdx) order — map
// iteration order would make the persisted cost basis depend on which side of
// a bad_debt each borrow lands. (TestBatchEquivalence covers the cross-ledger
// analogue; this is the same-ledger, cross-group case.)
//
// The ledger interleaves 10 borrows (amounts 1, 2, 4, ... 512) with 9
// bad_debts for the same key across 19 distinct (TxIdx, OpIdx) groups,
// chain-ordered borrow-first: only the final borrow (512) survives the last
// bad_debt. A map-order iteration nets 512 only when every other borrow
// happens to land before the last bad_debt (~2% per run); 8 runs with fresh
// processors drive an unordered implementation's pass probability below 1e-13.
func TestProcessLedger_FoldsEventGroupsInOpOrder(t *testing.T) {
	ctx := context.Background()
	poolAddr := randomContractAddr(t)
	asset := randomContractAddr(t)
	user := randomAccountAddr(t)
	...
	for i := 0; i < 19; i++ {
		key := indexer.ContractEventKey{TxIdx: uint32(i / 3), OpIdx: uint32(i % 3)}
		if i%2 == 0 {
			amount := int64(1) << (i / 2)
			events[key] = []xdr.ContractEvent{contractEvent(t, poolAddr,
				[]xdr.ScVal{symScVal("borrow"), contractAddrScVal(t, asset), accountAddrScVal(t, user)},
				vecScVal(i128ScVal(amount), i128ScVal(amount)),
			)}
		} else {
			events[key] = []xdr.ContractEvent{contractEvent(t, poolAddr,
				[]xdr.ScVal{symScVal("bad_debt"), accountAddrScVal(t, user), contractAddrScVal(t, asset)},
				i128ScVal(1),
			)}
		}
	}
	...
	for run := 0; run < 8; run++ {
		p, m := newFullTestProcessor(t)
		var cap captured
		wireCaptures(m.positions, &cap)
		require.NoError(t, p.ProcessLedger(ctx, input))
		require.NoError(t, p.PersistCurrentState(ctx, nil))

		require.Len(t, cap.netDeltaRows, 1, "run %d: the staged fold pre-aggregates to one row per key", run)
		row := cap.netDeltaRows[0]
		assert.True(t, row.ZeroBorrowed, "run %d: a bad_debt occurred, so the row must reset before adding", run)
		assert.Equal(t, "512", row.NetBorrowedDelta,
			"run %d: only the final borrow (tx 6, op 0) survives the last bad_debt (tx 5, op 2) in chain order", run)
	}
}
...
func TestNewProcessor(t *testing.T) {
	t.Run("nil Models leaves data-layer deps nil but still initializes staged sets", func(t *testing.T) {
		...
		assert.Equal(t, types.StateChangeOrdinalBaseBlend, p.StateChangeOrdinalBase())
