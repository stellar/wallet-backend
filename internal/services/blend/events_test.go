...
func TestParseEvent_Supply(t *testing.T) {
	...
	row := got.Rows[0]
	assert.Equal(t, types.StateChangeCategoryBlendSupply, row.Category)
	assert.Equal(t, types.StateChangeReasonCredit, row.Reason)
...
func TestParseEvent_SupplyCollateral(t *testing.T) {
	...
	require.Len(t, got.Rows, 1)
	row := got.Rows[0]
	assert.Equal(t, types.StateChangeCategoryBlendCollateral, row.Category)
	assert.Equal(t, types.StateChangeReasonCredit, row.Reason)
...
func TestParseEvent_WithdrawCollateral(t *testing.T) {
	...
	row := got.Rows[0]
	assert.Equal(t, types.StateChangeCategoryBlendCollateral, row.Category)
	assert.Equal(t, types.StateChangeReasonDebit, row.Reason)
	assert.Equal(t, assetAddr, row.Token)
...
func TestParseEvent_Borrow(t *testing.T) {
	...
	require.Len(t, got.Rows, 1)
	row := got.Rows[0]
	assert.Equal(t, types.StateChangeCategoryBlendDebt, row.Category)
...
func TestParseEvent_DefaultedDebt(t *testing.T) {
	...
	row := got.Rows[0]
	assert.Equal(t, types.StateChangeCategoryBlendDebt, row.Category)
	assert.Equal(t, types.StateChangeReasonBurn, row.Reason, "socialized debt burns the pool's dTokens")
...
func TestParseEvent_FillAuction(t *testing.T) {
	...
	t.Run("auction_type 0 (UserLiquidation) folds both user and filler", func(t *testing.T) {
		...
		assert.Equal(t, types.StateChangeCategoryBlendAuction, userRow.Category)
		assert.Equal(t, types.StateChangeReasonFill, userRow.Reason)
		...
		assert.Equal(t, types.StateChangeCategoryBlendAuction, fillerRow.Category)
		assert.Equal(t, types.StateChangeReasonFill, fillerRow.Reason)
	...
	t.Run("fill_percent range", func(t *testing.T) {
		auctionData := mapScVal(
			...
			symEntry("block", u32ScVal(999)),
			symEntry("lot", mapValScVal(mapScVal(addrMapEntry(t, assetA, i128ScVal(1000))))),
		)
		withFillPercent := func(raw xdr.ScVal) xdr.ContractEvent {
			return contractEvent(t, poolAddr,
				[]xdr.ScVal{symScVal("fill_auction"), u32ScVal(0), accountAddrScVal(t, userAddr)},
				vecScVal(accountAddrScVal(t, fillerAddr), raw, mapValScVal(auctionData)),
			)
		}

		t.Run("100 is a full fill and decodes", func(t *testing.T) {
			got, err := ParseEvent(withFillPercent(i128ScVal(100)), "", alwaysUntracked)
			require.NoError(t, err)
			require.NotNil(t, got)
			require.Len(t, got.Rows, 2)
			assert.Equal(t, 100, got.Rows[0].Extra["fillPercent"])
		})
		...
		for _, tc := range []struct {
			...
			{"zero", i128ScVal(0)},
			{"above 100", i128ScVal(101)},
			{"far above int32", i128ScVal(5_000_000_000)},
		} {
			t.Run(tc.name+" is rejected", func(t *testing.T) {
				_, err := ParseEvent(withFillPercent(tc.val), "", alwaysUntracked)
				require.Error(t, err)
				assert.Contains(t, err.Error(), "outside 1..100")
			})
		}
	})
}
...
func TestParseEvent_WithdrawDisambiguation(t *testing.T) {
	...
	t.Run("interprets as a backstop withdraw when topic[1] is a tracked Blend contract", func(t *testing.T) {
		...
		require.Len(t, got.Rows, 1)
		row := got.Rows[0]
		assert.Equal(t, types.StateChangeCategoryBlendBackstop, row.Category)
		assert.Equal(t, types.StateChangeReasonDebit, row.Reason)
...
func TestParseEvent_ClaimDisambiguation(t *testing.T) {
	...
	t.Run("pool claim: 2-tuple (Vec<u32>, i128) data", func(t *testing.T) {
		...
		require.Len(t, got.Rows, 1)
		row := got.Rows[0]
		assert.Equal(t, types.StateChangeCategoryBlendEmissions, row.Category)
...
func TestParseEvent_QueueWithdrawal(t *testing.T) {
	...
	require.Len(t, got.Rows, 1)
	row := got.Rows[0]
	assert.Equal(t, types.StateChangeCategoryBlendBackstopQueue, row.Category)
	assert.Equal(t, types.StateChangeReasonAdd, row.Reason)
	assert.Equal(t, fromAddr, row.Account)
	assert.Equal(t, "300", row.Amount)
	assert.Equal(t, poolAddr, row.PoolID)
...
func TestParseEvent_DequeueWithdrawal(t *testing.T) {
	...
	row := got.Rows[0]
	assert.Equal(t, types.StateChangeCategoryBlendBackstopQueue, row.Category)
	assert.Equal(t, types.StateChangeReasonRemove, row.Reason)
	assert.Equal(t, fromAddr, row.Account)
