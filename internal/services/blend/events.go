...
// into Blend state-change history rows plus cost-basis fold instructions.
...
// Blend history or cost-basis semantics.
...
// EventRow is one Blend state-change history row decoded from a Blend
// ContractEvent. Category names the on-chain object the row changes
// (BLEND_SUPPLY, BLEND_DEBT, ...) and Reason the action on it. Token == ""
// means the row's token_id column is NULL (BLEND_AUCTION fills, whose
// amounts are carried in Extra's lot/bid maps instead, and backstop
// share-denominated rows). Amount == "" means the row's amount column is
// NULL (BLEND_AUCTION fills). PoolID == "" means the row's poolId key_value
// entry is omitted (backstop emissions CLAIM, whose event carries no pool
// address at all).
...
func ParseEvent(event xdr.ContractEvent, blndToken string, isTracked func(addr string) bool) (*DecodedEvent, error) {
	...
	switch symName {
	case "supply":
		return decodeSupplyLike(symName, topics, data, poolID, types.StateChangeCategoryBlendSupply, types.StateChangeReasonCredit, "bTokens")
	case "supply_collateral":
		return decodeSupplyLike(symName, topics, data, poolID, types.StateChangeCategoryBlendCollateral, types.StateChangeReasonCredit, "bTokens")
	case "withdraw_collateral":
		return decodeWithdrawLike(symName, topics, data, poolID, types.StateChangeCategoryBlendCollateral, types.StateChangeReasonDebit, "bTokens")
	case "borrow":
...
func decodeSupplyLike(symName string, topics []xdr.ScVal, data xdr.ScVal, poolID string, category types.StateChangeCategory, reason types.StateChangeReason, protocolField string) (*DecodedEvent, error) {
	...
	row := EventRow{
		Category: category, Reason: reason, Account: from, Token: asset, Amount: tokensIn, PoolID: poolID,
...
// tokens_out.
func decodeWithdrawLike(symName string, topics []xdr.ScVal, data xdr.ScVal, poolID string, category types.StateChangeCategory, reason types.StateChangeReason, protocolField string) (*DecodedEvent, error) {
	asset, from, err := decodeTwoAddrTopics(topics, symName)
	if err != nil {
		return nil, err
	}
	...
	row := EventRow{
		Category: category, Reason: reason, Account: from, Token: asset, Amount: tokensOut, PoolID: poolID,
...
func decodeBorrowLike(symName string, topics []xdr.ScVal, data xdr.ScVal, poolID string, reason types.StateChangeReason, protocolField string, positive bool) (*DecodedEvent, error) {
	...
	row := EventRow{
		Category: types.StateChangeCategoryBlendDebt, Reason: reason, Account: from, Token: asset, Amount: amount, PoolID: poolID,
...
func decodeFlashLoan(topics []xdr.ScVal, data xdr.ScVal, poolID string) (*DecodedEvent, error) {
	...
	row := EventRow{
		Category: types.StateChangeCategoryBlendDebt, Reason: types.StateChangeReasonFlashLoan, Account: from, Token: asset, Amount: tokensOut, PoolID: poolID,
...
func decodeBadDebt(topics []xdr.ScVal, data xdr.ScVal, poolID string) (*DecodedEvent, error) {
	...
	row := EventRow{
		Category: types.StateChangeCategoryBlendDebt, Reason: types.StateChangeReasonBadDebt, Account: user, Token: asset, Amount: dTokens, PoolID: poolID,
...
func decodeDefaultedDebt(topics []xdr.ScVal, data xdr.ScVal, poolID string) (*DecodedEvent, error) {
	...
	row := EventRow{
		Category: types.StateChangeCategoryBlendDebt, Reason: types.StateChangeReasonBurn, Account: poolID, Token: asset, Amount: dTokensBurnt, PoolID: poolID,
...
func decodeFillAuction(topics []xdr.ScVal, data xdr.ScVal, poolID string) (*DecodedEvent, error) {
	...
	fillPercentVal, convErr := strconv.ParseInt(fillPercentStr, 10, 64)
	if convErr != nil {
		return nil, fmt.Errorf("blend: fill_auction: fill_percent %q is not an integer: %w", fillPercentStr, convErr)
	}
	// The pool contract panics on fill_percent outside 1..100, and events reach
	// this decoder only from wasm-hash-validated Blend pools, so a value outside
	// that range is junk rather than a protocol variant: reject the event.
	if fillPercentVal < 1 || fillPercentVal > 100 {
		return nil, fmt.Errorf("blend: fill_auction: fill_percent %d outside 1..100", fillPercentVal)
	}
	fillPercent := int(fillPercentVal)
	...
	// has no Blend history or cost-basis role and is not decoded.
	...
	rows := []EventRow{
		{Category: types.StateChangeCategoryBlendAuction, Reason: types.StateChangeReasonFill, Account: user, PoolID: poolID, Extra: extraFor(filler)},
		{Category: types.StateChangeCategoryBlendAuction, Reason: types.StateChangeReasonFill, Account: filler, PoolID: poolID, Extra: extraFor(user)},
	}
...
func decodeWithdrawAmbiguous(topics []xdr.ScVal, data xdr.ScVal, poolID string, isTracked func(addr string) bool) (*DecodedEvent, error) {
	...
	if isTracked(addr1) {
		...
		row := EventRow{
			Category: types.StateChangeCategoryBlendBackstop, Reason: types.StateChangeReasonDebit, Account: from, Token: "", Amount: tokensOut, PoolID: addr1,
	...
	row := EventRow{
		Category: types.StateChangeCategoryBlendSupply, Reason: types.StateChangeReasonDebit, Account: from, Token: addr1, Amount: tokensOut, PoolID: poolID,
...
func decodeClaimAmbiguous(topics []xdr.ScVal, data xdr.ScVal, poolID string, blndToken string) (*DecodedEvent, error) {
	...
	case xdr.ScValTypeScvI128:
		...
		row := EventRow{
			Category: types.StateChangeCategoryBlendBackstopEmissions, Reason: types.StateChangeReasonClaim, Account: from, Token: blndToken, Amount: amount, PoolID: "",
		}
		return &DecodedEvent{Rows: []EventRow{row}}, nil
	default:
...
func decodeBackstopDeposit(topics []xdr.ScVal, data xdr.ScVal) (*DecodedEvent, error) {
	...
	row := EventRow{
		Category: types.StateChangeCategoryBlendBackstop, Reason: types.StateChangeReasonCredit, Account: from, Token: "", Amount: tokensIn, PoolID: poolAddr,
...
func decodeQueueWithdrawal(topics []xdr.ScVal, data xdr.ScVal) (*DecodedEvent, error) {
	...
	row := EventRow{
		Category: types.StateChangeCategoryBlendBackstopQueue, Reason: types.StateChangeReasonAdd, Account: from, Token: "", Amount: amount, PoolID: poolAddr,
...
func decodeDequeueWithdrawal(topics []xdr.ScVal, data xdr.ScVal) (*DecodedEvent, error) {
	...
	row := EventRow{
		Category: types.StateChangeCategoryBlendBackstopQueue, Reason: types.StateChangeReasonRemove, Account: from, Token: "", Amount: amount, PoolID: poolAddr,
	}
	return &DecodedEvent{Rows: []EventRow{row}}, nil
}
