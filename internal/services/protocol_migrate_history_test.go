...
// each tracker.
func simulateHandoffOnMiss(backend *multiLedgerBackend, dbPool *pgxpool.Pool, protocolIDs []string) func(uint32) {
...
func TestProtocolMigrateHistory(t *testing.T) {
	...
	t.Run("context cancelled during fetch — returns context error, status failed", func(t *testing.T) {
		...
		_, err := dbPool.Exec(parentCtx, `INSERT INTO protocols (id, classification_status) VALUES ('testproto', 'success') ON CONFLICT (id) DO UPDATE SET classification_status = 'success'`)
		require.NoError(t, err)
