...
func getCurrentStateSentinel(t *testing.T, ctx context.Context, dbPool db.ConnectionPool, protocolID string, seq uint32) (uint32, bool) {
	t.Helper()
	var val uint32
	err := dbPool.GetContext(ctx, &val, `SELECT value FROM ingest_store WHERE key = $1`, fmt.Sprintf("test_%s_current_state_%d", protocolID, seq))
	if err != nil {
		return 0, false
	}
	return val, true
