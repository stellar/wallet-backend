	...
	testCases := []struct {
		...
		},
		{
			...
			setupMocks: func(t *testing.T, tokenIngestionService *TokenIngestionServiceMock, contractMetadataSvc *ContractMetadataServiceMock) {
				...
				}), []types.ContractChange{}, mock.MatchedBy(func(m map[string]types.AccountChange) bool { return true })).Return(fmt.Errorf("db error"))
			},
			wantErr:         true,
	...
	testCases := []struct {
		...
		},
		{
			...
			setupMocks: func(t *testing.T, tokenIngestionService *TokenIngestionServiceMock, backendFactory *func(ctx context.Context) (ledgerbackend.LedgerBackend, error)) {
				...
				tokenIngestionService.On("ProcessTokenChanges", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(fmt.Errorf("db connection error"))
