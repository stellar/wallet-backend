...
type BatchChanges struct {
	...
	ProtocolContractsByID     map[string]data.ProtocolContract
}
...
func (m *ingestService) startBackfilling(ctx context.Context, startLedger, endLedger uint32, mode BackfillMode) error {
	...
	if mode.isCatchup() {
		...
		mergedProtocolContracts := make(map[string]data.ProtocolContract)
...
func (m *ingestService) processBatchChanges(
	...
	protocolContracts map[string]data.ProtocolContract,
) error {
