...
func TestEffects_ProcessTransaction(t *testing.T) {
	t.Run("SetOption", func(t *testing.T) {
		...
		for _, change := range changes {
			assert.Equal(t, toid.New(12345, 1, 1).ToInt64(), change.ToID)
			assert.Equal(t, uint32(12345), change.GetLedgerNumber())
	...
	t.Run("SetTrustlineFlags - generates balance authorization state changes", func(t *testing.T) {
		...
		assert.Equal(t, toid.New(12345, 1, 1).ToInt64(), changes[0].ToID)
		assert.Equal(t, uint32(12345), changes[0].GetLedgerNumber())
	...
	t.Run("Sponsorship - reserves sponsorship created/updated/revoked", func(t *testing.T) {
		...
		assert.Equal(t, toid.New(12345, 1, 1).ToInt64(), changes[1].ToID)
		assert.Equal(t, uint32(12345), changes[1].GetLedgerNumber())
		...
		assert.Equal(t, types.StellarAddress("GACMZD5VJXTRLKVET72CETCYKELPNCOTTBDC6DHFEUPLG5DHEK534JQX"), changes[1].AccountID)
		...
		assert.Equal(t, types.StellarAddress("GBRPYHIL2CI3FNQ4BXLFMNDLFJUNPU2HY3ZMFSHONUCEOASW7QC7OX2H"), changes[2].AccountID)
		...
		assert.Equal(t, types.StellarAddress("GACMZD5VJXTRLKVET72CETCYKELPNCOTTBDC6DHFEUPLG5DHEK534JQX"), changes[3].AccountID)
		...
		assert.Equal(t, types.StellarAddress("GAHK7EEG2WWHVKDNT4CEQFZGKF2LGDSW2IVM4S5DP42RBW3K6BTODB4A"), changes[4].AccountID)
		...
		assert.Equal(t, types.StellarAddress("GBRPYHIL2CI3FNQ4BXLFMNDLFJUNPU2HY3ZMFSHONUCEOASW7QC7OX2H"), changes[5].AccountID)
		...
		assert.Equal(t, types.StellarAddress("GBRPYHIL2CI3FNQ4BXLFMNDLFJUNPU2HY3ZMFSHONUCEOASW7QC7OX2H"), changes[6].AccountID)
