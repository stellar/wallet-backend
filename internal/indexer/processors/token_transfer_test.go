...
func TestTokenTransferProcessor_Process(t *testing.T) {
	...
	t.Run("CreateAccount - extracts state changes for successful account creation", func(t *testing.T) {
		...
		require.Equal(t, accountA.ToAccountId().Address(), changes[1].CreatorAccountID.String())
	...
	t.Run("Contract events - SAC transfer still produces debit/credit state changes", func(t *testing.T) {
		// USDC is a valid SEP-11 asset code, so the SDK's SAC validation succeeds and the
		// resulting event carries an asset whose deterministic SAC contract ID matches the
		// event's contract — TokenTransferProcessor classifies it as SAC and emits rows.
		admin := keypair.MustRandom().Address()
		asset := xdr.MustNewCreditAsset("USDC", admin)
	...
	t.Run("Contract events - non-SAC SEP-41 / unclassified contracts are skipped", func(t *testing.T) {
		// Custom SEP-41 contracts (and any contract the SDK can't deterministically tie to a
		// SAC asset) are owned by their protocol processor (sep41.Processor) — TokenTransferProcessor
		// must not emit state changes for them, otherwise rows would be written twice. The asset
		// code here contains an underscore, which fails the SDK's SEP-11 asset validation, so the
		// SDK leaves the event's asset unset and TokenTransferProcessor sees ContractTypeUnknown.
		admin := keypair.MustRandom().Address()
		...
		processor := NewTokenTransferProcessor(networkPassphrase, nil)
		stateChanges := processTransaction(t, processor, tx)
		requireEventCount(t, stateChanges, 1)
		assertFeeEvent(t, stateChanges[0], "1234")
	})
}
