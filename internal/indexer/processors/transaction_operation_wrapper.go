...
func (operation *TransactionOperationWrapper) Details() (map[string]interface{}, error) {
	...
	case xdr.OperationTypeInvokeHostFunction:
		...
		case xdr.HostFunctionTypeHostFunctionTypeInvokeContract:
			...
			if err != nil {
				return nil, fmt.Errorf("getting contract address string: %w", err)
			}
...
func addLedgerKeyDetails(result map[string]interface{}, ledgerKey xdr.LedgerKey) error {
	...
	default:
		// Other ledger entry types (ContractData, ContractCode, ConfigSetting, Ttl) are not handled
	}
	return nil
}
...
func getLedgerKeyParticipants(ledgerKey xdr.LedgerKey) []xdr.AccountId {
	...
	default:
		// Other ledger entry types (LiquidityPool, ContractData, ContractCode, ConfigSetting, Ttl) have no participants
	}
	return result
}
