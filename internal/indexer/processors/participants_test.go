...
func TestParticipantsProcessor_GetOperationsParticipants(t *testing.T) {
	testCases := []struct {
		...
		wantParticipantsFn func(t *testing.T, op xdr.Operation) map[int64]OperationParticipants
		wantErrContains    string
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := xdr.SafeUnmarshalBase64(tc.ledgerCloseMetaXDR, &lcm)
			require.NoError(t, err)
			...
			op, ok := ingestTx.GetOperation(0)
			require.True(t, ok)
			assert.Equal(t, tc.wantParticipantsFn(t, op), gotParticipants)
		})
	}
}
