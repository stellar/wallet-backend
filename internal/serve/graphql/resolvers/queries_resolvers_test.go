...
func TestQueryResolver_TransactionByHash(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		assert.Equal(t, []byte("envelope1"), tx.EnvelopeXDR)
		assert.Equal(t, []byte("result1"), tx.ResultXDR)
		require.NotNil(t, tx.MetaXDR)
		assert.Equal(t, []byte("meta1"), tx.MetaXDR)
...
func TestQueryResolver_Operations(t *testing.T) {
	...
	t.Run("get operations with first/after limit and cursor", func(t *testing.T) {
		...
		assert.Equal(t, []byte("opxdr4"), ops.Edges[0].Node.OperationXDR)
		assert.Equal(t, []byte("opxdr5"), ops.Edges[1].Node.OperationXDR)
		assert.Equal(t, []byte("opxdr6"), ops.Edges[2].Node.OperationXDR)
		assert.Equal(t, []byte("opxdr7"), ops.Edges[3].Node.OperationXDR)
		assert.Equal(t, []byte("opxdr8"), ops.Edges[4].Node.OperationXDR)
	...
	t.Run("get operations with last/before limit and cursor", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Len(t, ops.Edges, 1)
		assert.Equal(t, []byte("opxdr6"), ops.Edges[0].Node.OperationXDR)
		assert.True(t, ops.PageInfo.HasNextPage)
		assert.True(t, ops.PageInfo.HasPreviousPage)
		...
		assert.Equal(t, []byte("opxdr5"), ops.Edges[4].Node.OperationXDR)
		assert.True(t, ops.PageInfo.HasNextPage)
		assert.False(t, ops.PageInfo.HasPreviousPage)
	})
...
func TestQueryResolver_OperationByID(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		assert.Equal(t, []byte("opxdr1"), op.OperationXDR)
