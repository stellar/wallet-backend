...
func NewResolver(models *data.Models, rpcService services.RPCService, balanceReader BalanceReader, m *metrics.Metrics, config ResolverConfig, simulationService services.TransactionSimulationService) *Resolver {
	return &Resolver{
		models:            models,
...
// Returns an error when the field is null, since the GraphQL schema declares it non-nullable.
//
//nolint:unparam // fieldName mirrors the other resolveRequired* helpers; every signer field happens to share one name.
