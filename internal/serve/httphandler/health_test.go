...
func TestHealthHandler_GetHealth(t *testing.T) {
	...
	t.Run("healthy - RPC and backend in sync", func(t *testing.T) {
		...
		_, err := dbConnectionPool.Pool().Exec(ctx, "DELETE FROM ingest_store WHERE key = 'latest_ingest_ledger'")
		require.NoError(t, err)
		_, err = dbConnectionPool.Pool().Exec(ctx, "INSERT INTO ingest_store (key, value) VALUES ('latest_ingest_ledger', $1)", uint32(98))
		require.NoError(t, err)
		...
		_, cleanupErr := dbConnectionPool.Pool().Exec(ctx, "DELETE FROM ingest_store WHERE key = 'latest_ingest_ledger'")
	...
	t.Run("unhealthy - backend significantly behind", func(t *testing.T) {
		...
		_, err := dbConnectionPool.Pool().Exec(ctx, "DELETE FROM ingest_store WHERE key = 'latest_ingest_ledger'")
		require.NoError(t, err)
		_, err = dbConnectionPool.Pool().Exec(ctx, "INSERT INTO ingest_store (key, value) VALUES ('latest_ingest_ledger', $1)", uint32(900))
		require.NoError(t, err)
