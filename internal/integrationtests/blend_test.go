...
// countBlendStateChanges counts every Blend state change, across all eight
// BLEND_* categories (BLEND_SUPPLY, BLEND_COLLATERAL, BLEND_DEBT,
// BLEND_AUCTION, BLEND_EMISSIONS, BLEND_BACKSTOP, BLEND_BACKSTOP_QUEUE,
// BLEND_BACKSTOP_EMISSIONS). The `\_` escapes LIKE's single-character
// wildcard so only the literal prefix matches.
func (s *BlendMigrationTestSuite) countBlendStateChanges(ctx context.Context, pool *pgxpool.Pool) int {
	var count int
	err := pool.QueryRow(ctx, `SELECT COUNT(*) FROM state_changes WHERE state_change_category LIKE 'BLEND\_%'`).Scan(&count)
	s.Require().NoError(err)
	return count
}

// countBlendCategoryReason counts one account's state changes carrying an
// exact (category, reason) pair.
func (s *BlendMigrationTestSuite) countBlendCategoryReason(ctx context.Context, pool *pgxpool.Pool, address, category, reason string) int {
	var count int
	err := pool.QueryRow(ctx,
		`SELECT COUNT(*) FROM state_changes WHERE account_id = $1 AND state_change_category = $2 AND state_change_reason = $3`,
		s.addressBytes(address), category, reason).Scan(&count)
	s.Require().NoError(err)
	return count
}

// assertAllBlendPoolIDs asserts every BLEND_* state-change row's
// key_value->>'poolId' equals poolID — the phase-1 fixture only ever produces
// pool-scoped rows (no backstop emissions CLAIM, whose poolId is NULL), so
// every row at this point in the run must agree.
func (s *BlendMigrationTestSuite) assertAllBlendPoolIDs(ctx context.Context, pool *pgxpool.Pool, poolID string) {
	var mismatches int
	err := pool.QueryRow(ctx,
		`SELECT COUNT(*) FROM state_changes WHERE state_change_category LIKE 'BLEND\_%' AND (key_value->>'poolId') IS DISTINCT FROM $1`,
		poolID).Scan(&mismatches)
	s.Require().NoError(err)
	s.Assert().Zero(mismatches, "every phase-1 Blend state change should carry poolId=%s", poolID)
}
...
func (s *BlendMigrationTestSuite) TestProtocolSetupThenMigration() {
	...
	// ------------------------------------------------------------------
	classifiedContracts, err := models.ProtocolContracts.GetByProtocolID(ctx, pool, blendProtocolID)
	s.Require().NoError(err)
	...
	s.Assert().Zero(s.countBlendStateChanges(ctx, pool), "no Blend state changes should exist before migration runs")
	...
	// state_changes: Blend counts by (account, category, reason).
	s.Assert().Equal(2, s.countBlendCategoryReason(ctx, pool, stack.Supplier.Address(), "BLEND_SUPPLY", "CREDIT"))
	s.Assert().Equal(1, s.countBlendCategoryReason(ctx, pool, stack.Supplier.Address(), "BLEND_COLLATERAL", "CREDIT"))
	s.Assert().Equal(1, s.countBlendCategoryReason(ctx, pool, stack.Borrower.Address(), "BLEND_COLLATERAL", "CREDIT"))
	s.Assert().Equal(1, s.countBlendCategoryReason(ctx, pool, stack.Borrower.Address(), "BLEND_DEBT", "BORROW"))
	s.Assert().Equal(1, s.countBlendCategoryReason(ctx, pool, stack.Borrower.Address(), "BLEND_DEBT", "REPAY"))
	s.Assert().Equal(1, s.countBlendCategoryReason(ctx, pool, stack.Whale.Address(), "BLEND_BACKSTOP", "CREDIT"))

	s.assertAllBlendPoolIDs(ctx, pool, stack.PoolID)

	// ------------------------------------------------------------------
	// Phase 5: protocol-migrate history --rebuild repairs both corruption
	// shapes — a row with a wrong value and a missing row — over exactly
	// the ledger range it is given.
	// ------------------------------------------------------------------
	s.runHistoryRebuild(ctx, pool, models, stack)
}
...
func (s *BlendMigrationTestSuite) runHistoryRebuild(ctx context.Context, pool *pgxpool.Pool, models *data.Models, stack *infrastructure.BlendStack) {
	...
	const corruptAmount = "999999999999"
	s.Require().NotEqual(corruptAmount, repayAmount)
	tag, err := pool.Exec(ctx,
		`UPDATE state_changes SET amount = $2
		 WHERE account_id = $1 AND state_change_category = 'BLEND_DEBT' AND state_change_reason = 'REPAY'`,
		s.addressBytes(stack.Borrower.Address()), corruptAmount)
	s.Require().NoError(err)
	s.Require().EqualValues(1, tag.RowsAffected())

	// And punch a hole — the missing-row shape (events never decoded at all).
	tag, err = pool.Exec(ctx,
		`DELETE FROM state_changes
		 WHERE account_id = $1 AND state_change_category = 'BLEND_DEBT' AND state_change_reason = 'BORROW'`,
		s.addressBytes(stack.Borrower.Address()))
	s.Require().NoError(err)
	s.Require().EqualValues(1, tag.RowsAffected())
	...
	s.Require().NoError(err)
	s.Require().Zerof(exitCode, "protocol-migrate history --rebuild should exit 0 (see `docker logs wallet-backend-blend-protocol-migrate-history-rebuild`); logs:\n%s", logs)

	// Both corruption shapes are repaired, and the table as a whole is where
	// it started: the rebuild re-derived its range rather than duplicating or
	// dropping rows.
	s.Assert().Equal(1, s.countBlendCategoryReason(ctx, pool, stack.Borrower.Address(), "BLEND_DEBT", "BORROW"),
		"the rebuild should have re-derived the deleted BORROW row")
	var repairedAmount string
	err = pool.QueryRow(ctx,
		`SELECT amount FROM state_changes
		 WHERE account_id = $1 AND state_change_category = 'BLEND_DEBT' AND state_change_reason = 'REPAY'`,
		s.addressBytes(stack.Borrower.Address())).Scan(&repairedAmount)
	s.Require().NoError(err)
	s.Assert().Equal(repayAmount, repairedAmount,
		"the rebuild should have replaced the corrupted REPAY amount with the re-derived value")
	s.Assert().Equal(blendBefore, s.countBlendStateChanges(ctx, pool))
	s.Assert().Equal(nonBlendBefore, s.countNonBlendStateChanges(ctx, pool),
		"a Blend history rebuild must not touch other namespaces' state changes")
	...
	s.Require().NoError(err)
	s.Require().Len(protocols, 1)
	s.Assert().Equal(data.StatusSuccess, protocols[0].HistoryMigrationStatus)
	s.Assert().Equal(data.StatusSuccess, protocols[0].CurrentStateMigrationStatus)
}

// countNonBlendStateChanges counts the state changes owned by every namespace
// other than Blend's — the rows a Blend rebuild must leave untouched.
func (s *BlendMigrationTestSuite) countNonBlendStateChanges(ctx context.Context, pool *pgxpool.Pool) int {
	var count int
	err := pool.QueryRow(ctx, `SELECT COUNT(*) FROM state_changes WHERE state_change_category NOT LIKE 'BLEND\_%'`).Scan(&count)
	s.Require().NoError(err)
	return count
}
...
func (s *BlendLiveIngestionTestSuite) assertPoolCatalog(p wbtypes.BlendPool, stack *infrastructure.BlendStack) {
	...
	// The stack's setup calls backstop.add_reward for this pool, so the backstop's reward-zone
	// entry lists it and blend_pools.in_reward_zone is set from that entry.
	s.Assert().True(p.InRewardZone, "the stack adds the pool to the backstop reward zone")
...
func (s *BlendLiveIngestionTestSuite) assertSupplierPositions(positions *wbtypes.BlendAccountPositions, stack *infrastructure.BlendStack) {
	...
	s.Assert().Empty(positions.ActiveAuctions, "the supplier is never an auction owner")
}
...
func (s *BlendLiveIngestionTestSuite) assertBorrowerPositions(positions *wbtypes.BlendAccountPositions, stack *infrastructure.BlendStack) {
	...
	// The fixture fills only BlendAuctionFillPercent of the liquidation auction, so the
	// borrower — the auction owner — still carries the remainder as an active auction.
	s.Require().Len(positions.ActiveAuctions, 1, "a partially filled liquidation auction stays active")
	auction := positions.ActiveAuctions[0]
	s.Assert().Equal(stack.PoolID, auction.PoolAddress)
	s.Require().NotNil(auction.PoolName)
	s.Assert().Equal(stack.PoolName, *auction.PoolName)
	s.Assert().Equal("USER_LIQUIDATION", auction.AuctionType)
	s.Assert().Greater(auction.StartBlock, int32(0))
	s.Assert().NotEmpty(auction.Lot, "the remaining lot is the borrower's collateral bTokens")
	s.Assert().NotEmpty(auction.Bid, "the remaining bid is the borrower's debt dTokens")
	for _, side := range [][]wbtypes.BlendAuctionAmount{auction.Lot, auction.Bid} {
		for _, amt := range side {
			s.Assert().NotEmpty(amt.AssetContractID)
			s.Assert().Greater(parseBigIntStr(s.T(), amt.Amount).Sign(), 0)
		}
	}
}
