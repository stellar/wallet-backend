...
func TestInt128MagnitudeBytesOverflowFails(t *testing.T) {
	huge := new(big.Int).Lsh(big.NewInt(1), 129) // far outside the signed range
	_, err := int128MagnitudeBytes(huge)
	require.Error(t, err)
}

// The valid domain is the SIGNED i128 range [-2^127, 2^127-1]: a magnitude in
// (2^127-1, 2^128-1] still fits 16 bytes but would flip the sign bit and
// silently encode a different (negative) number, so both boundaries and their
// first out-of-range neighbors are pinned here.
func TestInt128MagnitudeBytesSignedRangeBoundaries(t *testing.T) {
	...
	buf, err := int128MagnitudeBytes(maxI128)
	require.NoError(t, err)
	require.Equal(t, byte(0x7F), buf[0], "2^127-1 keeps the sign bit clear")
	for _, b := range buf[1:] {
		require.Equal(t, byte(0xFF), b)
	}

	buf, err = int128MagnitudeBytes(minI128)
	require.NoError(t, err)
	require.Equal(t, byte(0x80), buf[0], "-2^127 is exactly the sign bit")
	for _, b := range buf[1:] {
		require.Equal(t, byte(0x00), b)
	}

	_, err = int128MagnitudeBytes(new(big.Int).Add(maxI128, big.NewInt(1)))
	require.Error(t, err, "2^127 would reinterpret as -2^127")

	_, err = int128MagnitudeBytes(new(big.Int).Sub(minI128, big.NewInt(1)))
	require.Error(t, err, "-2^127-1 would reinterpret as 2^127-1")

	maxUint128 := new(big.Int).Sub(new(big.Int).Lsh(big.NewInt(1), 128), big.NewInt(1))
	_, err = int128MagnitudeBytes(maxUint128)
	require.Error(t, err, "2^128-1 fits 16 bytes but would encode -1")
}
