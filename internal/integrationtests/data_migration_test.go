...
func (s *DataMigrationTestSuite) setupDB() (db.ConnectionPool, func()) {
	ctx := context.Background()
...
func (s *DataMigrationTestSuite) setupModels(pool db.ConnectionPool) *data.Models {
...
func (s *DataMigrationTestSuite) resetDataMigrationState(ctx context.Context, pool db.ConnectionPool) {
	_, err := pool.ExecContext(ctx, `UPDATE protocol_wasms SET protocol_id = NULL`)
	s.Require().NoError(err)

	_, err = pool.ExecContext(ctx, `DELETE FROM protocols`)
	s.Require().NoError(err)
...
func (s *DataMigrationTestSuite) upsertIngestStoreValue(ctx context.Context, pool db.ConnectionPool, key string, value uint32) {
	_, err := pool.ExecContext(ctx,
...
func (s *DataMigrationTestSuite) ingestStoreKeyExists(ctx context.Context, pool db.ConnectionPool, key string) bool {
	var exists bool
	err := pool.GetContext(ctx, &exists, `SELECT EXISTS (SELECT 1 FROM ingest_store WHERE key = $1)`, key)
	s.Require().NoError(err)
	return exists
}

func (s *DataMigrationTestSuite) runSEP41ProtocolSetup(ctx context.Context, pool db.ConnectionPool, models *data.Models) {
...
func (s *DataMigrationTestSuite) newServiceMetricsMock() *metrics.MockMetricsService {
	mockMetrics := metrics.NewMockMetricsService()
	mockMetrics.On("RegisterPoolMetrics", mock.Anything, mock.Anything).Return().Maybe()
	mockMetrics.On("ObserveDBQueryDuration", mock.Anything, mock.Anything, mock.Anything).Return().Maybe()
	mockMetrics.On("ObserveDBBatchSize", mock.Anything, mock.Anything, mock.Anything).Return().Maybe()
	mockMetrics.On("IncDBQuery", mock.Anything, mock.Anything).Return().Maybe()
	mockMetrics.On("IncDBQueryError", mock.Anything, mock.Anything, mock.Anything).Return().Maybe()
	mockMetrics.On("SetLatestLedgerIngested", mock.Anything).Return().Maybe()
	mockMetrics.On("SetOldestLedgerIngested", mock.Anything).Return().Maybe()
	mockMetrics.On("ObserveIngestionDuration", mock.Anything).Return().Maybe()
	mockMetrics.On("ObserveIngestionPhaseDuration", mock.Anything, mock.Anything).Return().Maybe()
	mockMetrics.On("ObserveIngestionBatchSize", mock.Anything).Return().Maybe()
	mockMetrics.On("ObserveIngestionParticipantsCount", mock.Anything).Return().Maybe()
	mockMetrics.On("IncIngestionLedgersProcessed", mock.Anything).Return().Maybe()
	mockMetrics.On("IncIngestionTransactionsProcessed", mock.Anything).Return().Maybe()
	mockMetrics.On("IncIngestionOperationsProcessed", mock.Anything).Return().Maybe()
